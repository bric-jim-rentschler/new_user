...
class PackageIndex(Environment):
    """A distribution index.html that scans web pages for download URLs"""
    ...
    def process_index(self,url,page):
        ...
        def scan(link):
            ...
            if link.startswith(self.index_url):
                ...
                if len(parts)==2 and '#' not in parts[1]:
                    # it's a package page, sanitize and index.html it
        ...
        # process an index.html page into the package-page index.html
        for match in HREF.finditer(page):
            try:
        ...
        pkg, ver = scan(url)   # ensure this page is in the page index.html
        if pkg:
    ...
    def need_version_info(self, url):
        self.scan_all(
            "Page at %s links to .py file(s) without version info; an index.html "
    ...
    def scan_all(self, msg=None, *args):
        if self.index_url not in self.fetched_urls:
            ...
            self.info(
                "Scanning index.html of all packages (this may take a while)"
            )
    ...
    def find_packages(self, requirement):
        ...
        if not self.package_pages.get(requirement.key):
            # We couldn't find the target package, so search the index.html page too
    ...
    def not_found_in_index(self, requirement):
        if self[requirement.key]:   # we've seen at least one distro
            meth, msg = self.info, "Couldn't retrieve index.html page for %r"
        else:   # no distros seen for this name, might be misspelled
            meth, msg = (self.warn,
                "Couldn't find index.html page for %r (maybe misspelled?)")
    ...
    def fetch_distribution(
        ...
        searched for in the (online) package index.html as well as the locally
    ...
    def _download_html(self, url, headers, filename):
        ...
        for line in file:
            if line.strip():
                # Check for a subversion index.html page
                if re.search(r'<title>([^- ]+ - )?Revision \d+:', line):
                    # it's a subversion index.html page:
                    file.close()
                    os.unlink(filename)
                    return self._download_svn(url, filename)
                break   # not an index.html page
        file.close()
        os.unlink(filename)
