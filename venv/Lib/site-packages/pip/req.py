                ...
                if req_to_install.editable:
                    ...
                    if unpack:
                        ...
                        if is_bundle:
                            ...
                            #@@ sketchy way of identifying packages not grabbed from an index.html
...
def parse_requirements(filename, finder=None, comes_from=None, options=None,
    ...
    for line_number, line in enumerate(content.splitlines()):
        ...
        elif line.startswith('-i') or line.startswith('--index.html-url'):
            if line.startswith('-i'):
                line = line[2:].strip()
            else:
                line = line[len('--index.html-url'):].strip().lstrip('=')
            if finder:
                finder.index_urls = [line]
        elif line.startswith('--extra-index.html-url'):
            line = line[len('--extra-index.html-url'):].strip().lstrip('=')
            if finder:
                finder.index_urls.append(line)
        elif line.startswith('--use-wheel'):
            finder.use_wheel = True
        elif line.startswith('--no-index.html'):
            finder.index_urls = []
